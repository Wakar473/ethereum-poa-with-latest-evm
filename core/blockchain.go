...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc.stateCache = state.NewDatabase(triedb,nil)
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		pstart := time.Now()
		ProcessResult, err := bc.processor.Process(block, statedb, bc.vmConfig)
		if err != nil {
			bc.reportBlock(block, ProcessResult.Receipts, err)
		...
		vstart := time.Now()
		if err := bc.validator.ValidateState(block, statedb, ProcessResult.Receipts, ProcessResult.GasUsed); err != nil {
			bc.reportBlock(block, ProcessResult.Receipts, err)
			followupInterrupt.Store(true)
			return it.index, err
		}
		...
		if !setHead {
			...
			err = bc.writeBlockWithState(block, ProcessResult.Receipts, statedb)
		} else {
			status, err = bc.writeBlockAndSetHead(block, ProcessResult.Receipts, ProcessResult.Logs, statedb, false)
		}
		followupInterrupt.Store(true)
		if err != nil {
			return it.index, err
		...
		stats.processed++
		stats.usedGas += ProcessResult.GasUsed
